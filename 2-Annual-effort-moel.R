...
mod1 = felm(log(1 + effort) ~ sst + chlor_a + WSPD, data = regdat)
summary(mod1)

mod2 = felm(log(1 + effort) ~ sst + chlor_a + WSPD + cc_ratio, data = regdat)
summary(mod2)

mod3 = felm(log(1 + effort) ~ sst + chlor_a + WSPD + cc_ratio + hurricane, data = regdat)
summary(mod3)

mod4 = felm(log(1 + effort) ~ sst + chlor_a + WSPD + cc_ratio + hurricane | region, data = regdat)
summary(mod4)

mod5 = felm(log(1 + effort) ~ sst + chlor_a + WSPD + cc_ratio + hurricane | year + region, data = regdat)
summary(mod5)
...
stargazer(mod1, mod2, mod3, mod4, mod5, 
          ...
          covariate.labels = c("SST", "CHL", "Wind Speed", " Conflict/Coop Ratio", "Hurricane"),
          # apply.coef = multiply.100, apply.se = multiply.100,
          table.layout ="=dcm#-t-as=n",
          font.size = "footnotesize",
          add.lines = list(c("Region FE", "--", "--", "--", "Yes", "Yes"),
                           c("Year FE", "--", "--", "--", "--", "Yes")),
